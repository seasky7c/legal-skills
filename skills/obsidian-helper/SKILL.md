...
description: |
  Obsidian 智能笔记助手。当用户提到 obsidian、日记、笔记、知识库、capture、review、归档、链接 时激活。
  ...
  - 自动双向链接（auto-link）
  - 自动 Callouts 标注
  - 与 scrapling-web-extractor 整合归档（默认）
version: 1.6.0
author: Claude Code + 嘟嘟
---
...
| `/auto-link [文件]` | 自动双向链接 | 为笔记添加 Wikilinks |
| `/archive <URL>` | 知识归档 | 从 URL 归档文章到 Obsidian |
...
## 📋 功能四：/auto-link - 自动双向链接

### 触发条件
- 用户输入 `/auto-link` 或 `/auto-link <文件路径>`
- 用户说「添加双向链接」「自动链接」「auto link」
- 用户归档文章后，询问「是否添加双向链接」
...
用户可创建 `_config/auto-link.json` 自定义配置：

```json
{
  "whitelist": ["Obsidian", "AI", "大模型"],  // 只链接这些
  "blacklist": ["总结", "笔记", "模板"],      // 跳过这些
  "min_length": 2,                            // 最小标题长度
  "exclude_dirs": ["10-Articles", "99-Archives"]
}
```
...
**示例 1：处理单个文件**
```
用户：/auto-link 10-Articles/Web/Obsidian Skills 如何重塑知识工作流.md
```
Claude: 扫描 → 添加链接 → 报告结果
...
用户：/auto-link --all
```
Claude: 「这将处理所有笔记，可能需要几分钟。确认继续？」

**示例 3：预览模式**
```
用户：/auto-link <文件> --dry-run
```
Claude: 显示将添加哪些链接，不修改文件

---

## 📋 功能五：/archive - 知识归档（整合 scrapling-web-extractor）

### 触发条件
- 用户提供 URL 并要求「归档」「存入笔记」「保存到 Obsidian」
- 用户输入 `/archive <URL>`
...
STEP 2: 提取内容
├─ 调用 scrapling-web-extractor 技能（默认）
├─ 如果失败，备用 china-news-crawler
├─ 提取：标题、作者、发布时间、正文、图片
└─ 转换为 Markdown 格式

STEP 3: 自动处理
├─ 添加 Frontmatter（type: article, source, tags 等）
├─ 识别关键概念，添加 Wikilinks
├─ 用 Callouts 标注重点内容
└─ 生成摘要
...
**示例 1：归档微信公众号文章**
```
用户：https://mp.weixin.qq.com/s/xxx 归档到笔记
```
Claude: 提取 → 处理 → 保存 → 报告

**示例 2：归档并添加链接**
```
用户：/archive <URL> --auto-link
```
Claude: 归档后自动添加双向链接
